...
### 按档位诊断收官(2026-09-14 19:58):`13691373` COMPLETED 01:05:14

| 档位 | 结果 | gen / recompute |
|---|---|---|
| `auto`(bf16) | **OK** 18:53:14→19:25:24 | 4 / 2 |
| `fp8_e4m3` | **OK** 19:25:24→19:56:30 | 4 / 2 |
| `fp8_e5m2` | **FAIL** rc=1,约 2 分钟(非超时) | 0 / 0 |

**预登记判定(R83 定阈值于数据产出前,R84 记结果)**:

| H200 档位 | E[e^{2ε}]−1 | sd(ε) | min ε | ESS(W) |
|---|---|---|---|---|
| bf16 | 9.7455e−03 | 0.09637 | −8.986 | 2.86% |
| **fp8_e4m3** | **8.5487e−02** | **0.24815** | **−13.402** | **0.12%** |

`R = 8.7719`,逐分片 9.3074 / 8.2166 **同向** ⇒ **判定:显著剂量效应**。

**但比舍入模型预测低约 29 倍**(线性缩放预期 256 倍);
`sd(ε)` 仅涨 2.575 倍而舍入涨 16 倍,即 `sd ∝ u^0.34`,严重次线性。
两点拟两参数的"底噪+量化"分解**不可证伪**(需第三档,而 e5m2/fp4 在 vLLM 侧拿不到)。

**硬件效应:无可检出**(A100 vs H200 同批轨迹,逐分片比值 1.0491 / 0.9672 **方向相反**;
汇总的 1.0075 若单独引用会给出方向性假结论,见 R83)。

产出目录 `~/nobackup/autodelete/gap_measurement_kv/{auto,fp8_e4m3}/`,
基线 `~/nobackup/autodelete/gap_measurement/` 全程未被触碰(安全闸 + `KT_DATA_ROOT` 隔离)。
汇总脚本 `env_local/ladder_moments.py`(判定阈值已写死在脚本内)。


### 更正:两个 gstandby 臂的起始估计滑了 15 小时(2026-09-14 20:08 复核)

| 臂 | JobID | 我先前记录 | 现在 | 变化 |
|---|---|---|---|---|
| e5m2 / `fullis` | 13689470 | 09-14 21:40 | 09-14 21:40:07 | 未变 |
| e5m2 / `nocorr` | 13689486 | 09-16 07:00 | **09-16 22:00** | **+15 小时** |
| e5m2 / `ours` | 13689487 | 09-16 07:00 | **09-16 22:00** | **+15 小时** |

**机制(逐项查证,非推测)**:

```
13689470  Priority=100309  QOS=cs        Reason=Resources
13689486  Priority= 50309  QOS=gstandby  Reason=Priority
13689487  Priority= 50309  QOS=gstandby  Reason=Priority
```

- `cs` 比 `gstandby` **恰好高 50000** 优先级。
- `13689470` 的 `Reason=Resources` 表示它已排在最前、只等容量;
  另两臂的 `Reason=Priority` 表示被**更高优先级作业挡住** ——
  **其中就包括我自己的 `13689470`**。这正是 R74 那条坑的再次现形:
  **我自己的队列在给我自己制造延迟。**
- 此刻**没有任何节点有 8 张空卡**(cs-2-1 空 2、cs-2-2 空 3、eng-1-1 空 1、m13h 空 0–1),
  而 dose 臂需要**整节点 8 卡**。

**为什么不采取行动**:唯一能提前的办法是提到 `qos=cs`,
但先前两次探测都显示那会**抢占他人正在运行的作业**(fslcollab4 / dgl2),
今天已因此两次放弃该路径,此处同样不做。
降低卡数不可行 —— colocate 训练+推理需要整节点。

**一条应长期记住的读法**:`Reason=Priority` 的作业,
其 `StartTime` 是 Slurm 的**滚动估计**,三小时内可移动 15 小时。
**它是指示,不是 ETA**(承 R74:对已排队作业,调度器给的时刻不可当承诺)。

**时间表相应顺延**:完整报告的瓶颈由「9/16 晚」改为「**9/17**」,
且该日期同样随估计浮动。`13689470`(今晚 21:40)不受影响,
其五 benchmark 评测门控已挂(`ckpt_gate.sh` + epoch2 前缀校验)。


### 门控提交分支的事前验证(2026-09-14 20:1x)

**动机**:`ckpt_gate.sh` 先前的三次干跑,分别走了「tag 已存在 → 跳过」与
「epoch 前缀不匹配 → 超时」两条分支。**真正的提交分支从未执行过。**
它今晚约 03:00 无人值守地触发,若命令形式有误,会在最关键处静默失败
(正是 R58 记载的那类损失)。

**做法**:用与门控**逐字相同**的形式做 `--test-only`,只解析不提交:

```
sbatch --test-only --export=ALL,EVAL_PATH=<已完成臂的 epoch2>,EVAL_TAG=__gate_preflight__ \
       rl/eval_suite.sbatch
→ sbatch: Job 13692095 to start at 2026-09-15T14:04:27 ... on nodes dw-1-3 in partition dw
```

| 检查项 | 结果 |
|---|---|
| 命令形式 / `--export` 语法 | ✓ 被接受 |
| 分区 / QOS / gres 有效 | ✓ dw / dw87 / a100:4 |
| 是否抢占他人 | ✓ 无 `Preempts:` 行 |
| 是否真的没提交 | ✓ 前后均为 3 个作业 |
| `rl/eval_suite.py` | ✓ 存在(110 行) |
| 目标 tag 是否已存在 | ✓ `dose5_fp8_e5m2_fullis` 计数 0,无重复风险 |

**顺带发现的时间表修正**:`rl/eval_suite.sbatch` 投的是 **dw / 4×A100**,
而此刻新提交一个同规格作业的探测值是 **09-15 14:04**。
**⇒ 门控在 03:00 提交后,评测本身还要在 dw 排队。**
先前的时间表默认「训练完 → 评测结果紧随其后」,**这个默认不成立**。

**但不要把 14:04 当成 ETA**(承 R74):`--test-only` 回答的是
「现在再新增一个作业何时能起」,而 03:00 时 dw 的队列状态与现在不同。
准确的说法是:**评测会排队,排多久现在无法预测**。
门控的职责是「按时正确提交」,不是「保证评测立刻开跑」——
这一点已验证达成。


### 时间表再变:另两臂的起始估计提前约 41 小时(2026-09-15 00:1x)

| | 09-14 20:08 记录 | 09-15 00:11 实测 | 变化 |
|---|---|---|---|
| 13689486 | 09-16 22:00 | **09-15 05:01:23** | **提前约 41 小时** |
| 13689487 | 09-16 22:00 | **09-15 05:01:23** | 同上 |

`Reason` 仍为 `Priority`。**这是 R74 那条判据的又一次印证,且这次是朝有利方向**:
同一批作业的 `StartTime` 在约 4 小时内先后向后滑 15 小时、再向前跳 41 小时。
**对 `Reason=Priority` 的作业,该时刻是滚动估计,既不能当承诺、也不能当坏消息。**

`13689470` 进度正常:`epoch 目录=2/3` 于 00:12(早于我外推的 00:41),
epoch2 预计 01:5x–02:3x,门控预算至 09:06,余量充足。

**完整报告的预期因此由「9/17」上修为「9/15–9/16」**,但同样随估计浮动。


### `fp8_e5m2` 第一臂落地:`fullis` = 30.48(2026-09-15 04:5x)

| JobID | 内容 | 结果 |
|---|---|---|
| 13689470 | e5m2 / `fullis` 训练 | 3/3 epoch,实耗 **7:36+**(我先前按中段节奏外推 5:18,**低估约 3 小时**) |
| **13694563** | `eval_suite`(五 benchmark) | `dose5_fp8_e5m2_fullis` 等权 **30.48** |

逐项:gsm8k=60.50  math500=21.00  svamp=61.67  minerva=4.78  olympiad=4.45(`gsm8k_em`=60.35,不计入)

**门控首次真实触发并成功**:第 454 次检查捕获 `epoch2epochstep28globalstep86` 字节完整,
提交 `13694563`;该作业在 dw-2-3 上**立即开跑、未排队**。
tsv 落盘核验:**+6 行 / −0 行**,tag 正确。

#### `fullis` 沿阶梯的形状 —— **单 seed 下不可解读**

| 档位 | u | 等权均值 | 较上一档 |
|---|---|---|---|
| bf16 | 2⁻⁸ | 34.89 | — |
| fp8_e4m3 | 2⁻⁴ | 32.56 | -2.33 |
| **fp8_e5m2** | **2⁻³** | **30.48** | **-2.08** |
| fp4_e2m1 | 2⁻² | 32.32 | **+1.84** |

fp4 处的**非单调回升**很扎眼,但 **几乎可以确定不是真的**:
cell A 三种子的等权均值 SD 为 nocorr 0.22、ours 0.48、**fullis 2.23** ——
`fullis` 是三个算子里种子最脆的那个,而这里两步变化 -2.08 与 +1.84
**都落在它自己一个种子 SD(2.23)之内**。

**⇒ 本档在 `nocorr` / `ours` 落地前不作判读。**
`nocorr` 的种子 SD 只有 0.22(紧 10 倍),且它正是在 fp4 崩塌的那个算子,
这一档的信息量几乎全在它身上。

#### 一个结构性事实:这一档**永远不会有 ε 测量**

`fp8_e5m2` 在 vLLM 0.26 上不可用(R80:FlashAttention 拒收、FlashInfer warmup 崩)。
所以它**有结果轴、没有诊断轴**。加上 fp4 同样缺 ε,
**四档阶梯里有两档无法把结果与诊断配对** ——
这不是削弱 R85(sglang 打分)的理由,而是加强它的理由。


### e5m2 / `fullis` 旧协议评测收官(2026-09-15 05:3x)

| JobID | 内容 | 结果 |
|---|---|---|
| 13694660 | `eval_tp`(旧协议 GSM8K,**人工补投**) | `dose_fp8_e5m2_fullis` = **59.14**(780/1319),COMPLETED 00:06:12 |

人工补投的原因见 NOTES **R91**(收尾挂起,六道闸核验后处置)。
该臂两把量具:**旧协议 59.14 / 五 benchmark 等权 30.48**。

**`dose_curve.sh` 的 R90 修复已在实况下验证**:e5m2 行现渲染为
`54.36*  59.14  60.12*`,来源标注为
`!! 混合 1新/2既有 —— 带*者已作废(R59),禁止与同行比较`。
修复前该行会被整体标成「本次(Hopper+triton)」。


### e5m2 / `nocorr` 被抢占并重投(2026-09-15 06:4x)

| | JobID | 结局 |
|---|---|---|
| 初次 | ~~13689486~~ | **PREEMPTED** 06:39:27,跑 01:12:12,`epoch 0/3`,无产出 |
| 重投 | **13694731** | `qos=cs` @ cs-2-1,06:46:02 起跑,**不可被 gstandby/standby 抢占** |

重投选 `qos=cs` 的依据见 NOTES **R93**:探测**未报 `Preempts:` 行**
(cs-2-1 的 GPU 一张未占,dgl2 的作业各只要 1 CPU,可共存),
且提交后核实 dgl2 作业与我自己的 `13689487` 均未受影响。

`dose_jobids.txt` 已同步(13689486 → 13694731,备份 `.bak`)。
门控与守卫已重挂:预算至约 18:46,要求 `epoch2*` 前缀 + 字节完整 + tag 未入 tsv。
...
### 时间表更新(2026-09-15 18:2x):`ours` 臂受集群饱和阻塞

| 臂 | 状态 | 旧协议 | 五 benchmark |
|---|---|---|---|
| `fullis` | ✅ 完成 | 59.14 | 30.48 |
| `nocorr` | ✅ 完成(重投后 COMPLETED 04:28:55) | 53.83 | 29.92 |
| `ours` | ⏳ PENDING,估计 **09-16 18:00** | — | — |

`13694804` 自 07:43 排队至今未起。cs-2-1 于 11:15 被 nocorr 释放后,
立刻被 kzhao2 的 `cs` 作业接管;此刻 cs2/eng/m13h/cs3 **八个节点空闲 GPU 全为 0**。
所有替代落脚点均会挤占他人(详见 NOTES **R96**),故**维持现状等待**。

**完整报告的预期顺延至 9/17**(ours 起跑 → 约 4.5 小时训练 → 两次评测)。

**但已可确定的部分不受影响**:结论① 与 结论② 已由 `nocorr` + `fullis` 定稿
(剂量曲线拐点、修正起效阈值,均落在 e5m2 与 fp4 之间,且两把量具下形状一致)。
待 `ours` 的只有结论③(哪种修正更好)与 §5 的 CIS 主张核对。
